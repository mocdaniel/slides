...
September 3, 2026<br/>ContainerDays Hamburg
...
- Senior Developer Advocate at **NETWAYS Web Services**, a sovereign German cloud provider
- Main interests
  - Kubernetes
  - Observability
  - GitOps
...
Boot assets for all supported deployment scenarios are available on [factory.talos.dev](https://factory.talos.dev).
...
talosctl ls -t f -n 192.168.1.11 /usr/bin | tail -n +2 | awk '{print $2}' | paste -sd, -
  ...
  apparmor_parser,containerd,containerd-shim-runc-v2,cryptsetup,dmsetup,e2fsck,fatlabel,fsck.fat,init,
  integritysetup,lvm,mke2fs,mkfs.fat,mkfs.xfs,modprobe,nft,pigz,resize2fs,runc,tune2fs,udevadm,unpigz,
  veritysetup,xfs_copy,xfs_db,xfs_estimate,xfs_fsr,xfs_growfs,xfs_io,xfs_logprint,xfs_mdrestore,xfs_quota,
...
Commands executed 2026-09-02 on Talos v1.13.9.
...
- Five things to save/write down:
  - cluster name
  - cluster endpoint
  - `secrets.yaml`
  - Kubernetes version
